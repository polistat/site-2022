...
export default function ChancesTimeline({ dates, timeline, labels }: { dates: string[], timeline: number[], labels?: any }) {
  ...
  return <>
    ...
    >
      ...
      {/* lines */}
      <path
        className={`fill-transparent ${labels?.democrat==='Murkowski' ? 'stroke-red-500' : labels?.independent ? 'stroke-purple-500' : 'stroke-blue-500' } stroke-[4px]`}
      ...
      {selectedIndex!==null && <>
        ...
        <text
          ...
          className={`text-2xl font-semibold ${labels?.democrat==='Murkowski' ? 'fill-red-500' : labels?.independent ? 'fill-purple-500' : 'fill-blue-500' }`}
        >
